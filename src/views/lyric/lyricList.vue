<template>
	<div>
		...
		<LyricEdit :editID="editID" @editClose="editClose" v-if="showEdit"></LyricEdit>
	</div>
	</div>
</template>
...
<script>
	...
	import LyricEdit from './lyricEdit.vue'
	export default {
		components: {
			LinTable,
			LinSearch,
		...
		methods: {
			async getlyric(page, count) {
				...
				try {
					const lyrics = await Lyric.getLyrics(params)
					this.tableData = lyrics.data
					this.currentPage = lyrics.current_page
					this.total_nums = lyrics.total
					this.loading = false
				} catch (error) {
			...
			handleDelete(val) {
				...
				this.$confirm('此操作将永久删除该歌曲, 是否继续?', '提示', {
					...
					const res = await Lyric.delectLyric(id)
					if (res.error_code === 0) {
						this.getlyric(this.currentPage, this.pageCount)
						this.$message({
							type: 'success',
							message: `${res.msg}`
						})
		...
		async created() {
			await this.getlyric(this.currentPage, this.pageCount)
			this.operate = [{
					name: '编辑',
					func: 'handleEdit',
					type: 'primary'
