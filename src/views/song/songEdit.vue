<template>
	<div class="container">
		<div class="title">
			<span>修改歌曲</span> <span class="back" @click="back"> <i class="iconfont icon-fanhui"></i> 返回 </span>
		</div>
	...
	export default {
		...
		methods: {
			...
			async submitForm(formName) {
				...
				const res = await Song.editSong(this.form)
				if (res.error_code === 0) {
		...
		async created() {
			await this.getSong(this.editID)
			this.initUpload(this.form.img)
			if (this.form.from == 1) {
				this.form.play_url = src(this.form.play_url)
