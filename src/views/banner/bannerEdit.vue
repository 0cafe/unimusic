				...
				<el-col :lg="16" :md="20" :sm="24" :xs="24">
					<el-form :model="form" status-icon ref="form" label-width="200px" @submit.native.prevent v-loading="loading">
						...
						<el-form-item label="图片">
							<upload-imgs v-auth="{auth:'删除歌单',type:'disabled'}" ref="uploadEle" :value="initData" :multiple="true" :max-num="1" :auto-upload="true" />
						</el-form-item>
	...
	export default {
		...
		methods: {
			...
			async submitForm(formName) {
				...
				if (!this.form.img || !this.form.title) {
					this.loading = false
					return this.$message.error('请完整填写')
				}
				const res = await Banner.editBanner(this.form)
				if (res.error_code === 0) {
		...
		async created() {
			await this.getBanner(this.editID)
